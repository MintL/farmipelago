...
- cinematic two-island opening: an establishing view frames both parked vehicles on the stationary Settlement Island, reveals and follows the non-colliding Farm Island approaching from the south, then eases back to the normal drive camera after the shape-measured, center-aligned Farm and its roughly two-tile bridge activate together across a clear air gap; the HUD and gameplay input stay suppressed until that release
- continuous southwest travel presentation through a dense field of smaller northeast-moving distant voxel clouds and a much smaller population of larger near clouds that pass at island-edge and underside height about two and a half times faster; both use opaque, grid-aligned, deterministic stepped layouts wrapped around a stable two-island frame, while reduced motion retains slower steady translation without cloud bob or rapid near passes
- the Farm Island contains the starter field, south-coast lake, and a walk-in workshop built on the five-small-voxels-per-terrain-tile grid; the Settlement Island contains distinct tractor/combine spawns with reserved turnaround space, a compact non-interactive cluster of voxel-built homes, a communal receiving structure, worn paths, warm lighting, and no player farming or construction
- permanent cargo hub on the Settlement Island, with a cantilevered small-voxel landing deck, inlaid stepped markings, two bright hanging voxel lanterns, voxel-built staged cargo, and an articulated four-fan voxel VTOL; completing a delivery milestone summons it for a short celebration flight and unlock reveal
...
- a large irregular lake along the Farm Island's south coast with an east-flowing river and waterfall; landing in water throws flat-colored droplets using the same day/night base-water palette without surface glints
...
- ambient reindeer and foxes are temporarily absent from this two-island slice; cattle and their livestock progression remain available
...
- automatic browser-local saving of the opening lifecycle and generated Farmipelago, time of day, fields, crops, construction, livestock, progression, vehicles and inventories under the fresh schema-0 `farmipelago.gameState.v2` lineage; refresh during the intro replays the cinematic from the start, while refresh afterward restores the attached pair without replaying it, and the legacy `farmipelago.gameState` value is ignored and preserved for rollback or manual recovery
