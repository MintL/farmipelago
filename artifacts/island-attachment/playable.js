import { attachmentCameraSide, attachmentCameraFrame } from '/src/app/attachment-camera.js';
import { THREE } from '/src/core/shared.js';
import { crops } from '/src/gameplay/catalog/crops.js';
import { createPhysics } from '/src/physics/index.js';
import { createVehicle } from '/src/gameplay/vehicles/visual.js';
import { createLoadoutPreview } from '/src/gameplay/vehicles/loadout-preview.js';
import { createIslandSelectionView } from '/src/ui/island-selection.js';
import { createUi } from '/src/ui/index.js';
import { createBuildingManager } from '/src/gameplay/construction/index.js';
import { generateFarm, generateIsland } from '/src/world/generator.js';
import { createArchipelagoRuntime } from '/src/world/archipelago/runtime.js';
import { createSaveCoordinator } from '/src/app/save-coordinator.js';
import { createVillageNeeds } from '/src/gameplay/progression/index.js';
import { loadGameState } from '/src/persistence/index.js';
import { OWNED_VEHICLES, vehicleType } from '/src/gameplay/catalog/vehicles.js';
import { BALER_STORAGE_CAPACITY, equipmentDefinition, normalizeLoadout } from '/src/gameplay/catalog/equipment.js';
import { HAY_BALE_LITRES } from '/src/gameplay/livestock/index.js';
import { createEnvironment, DEFAULT_DAY_PHASE } from '/src/world/environment/index.js';
import { createTravelModel, travelFrameForIslands } from '/src/world/travel.js';
import { createTransferEffects } from '/src/gameplay/logistics/transfer-effects.js';
import { createTransferController } from '/src/gameplay/logistics/transfer-controller.js';
...
function persistentState() {
  ...
  return {
    ...
    vehicles: fleet.map(vehicle => {
      ...
      // Encounters restart on reload; keep cargo/loadout, restore the vehicle
      // to its own permanent spawn instead of saving a vanished island pose.
...
function updateDriveCamera(state, dt, snap = false) {
  const preview = farm?.attachments.previewFrame();
  if (preview && !openingCinematic) {
    const target = new THREE.Vector3(preview.x, preview.y, preview.z);
    const distance = preview.radius / Math.sin(THREE.MathUtils.degToRad(camera.fov / 2)) / Math.min(1, camera.aspect);
    const goal = target.clone().add(driveCameraOffset.clone().normalize().multiplyScalar(distance));
    const blend = snap ? 1 : 1 - Math.exp(-3 * dt);
    camera.position.lerp(goal, blend);
    driveCameraTarget.lerp(target, blend);
    camera.lookAt(driveCameraTarget);
    setCameraFogScale(Math.max(1, distance / 32));
    return;
  }
...
function initializeFarm(savedState) {
  ...
  farm = createArchipelagoRuntime(generateFarm(
    ...
    0,
    scheduleSave,
    { attachmentComplete, reducedMotion, camera, savedWorld: savedState?.world,
      beforeIslandDetach, afterIslandAttach },
  ));
...
initializeFarm({ world: { seed: 99173, connections: [{status: 'attached'}] }, buildings: [], vehicles: [], ui: {}, progression: {} });
