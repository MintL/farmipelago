...
export function findVehicleSpawnPoints(terrain, start, islandId, workshopSite = null) {
  ...
  const candidates = [...terrain.values()].filter(tile =>
    tile.islandId === islandId && !tile.water && !tile.hasTree && !tile.reserved &&
  ...
  if (selected.length < 2) throw new Error(`Island ${islandId} requires distinct tractor and harvester spawn points`);
  return {
    tractor: selected[0],
...
export function reserveVehicleSpawnGround(terrain, spawnPoints) {
  for (const tile of terrain.values()) {
    const distance = Math.min(...Object.values(spawnPoints)
      .map(spawn => Math.hypot(tile.x - spawn.x, tile.z - spawn.z)));
    if (distance <= TILE * 2.25) {
      tile.noDecoration = true;
      if (distance <= TILE * 1.45) tile.reserved = true;
    }
  }
}
