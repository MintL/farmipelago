...
- a second, round build button beneath suitability: open it to enter a pannable construction view and place grain silos or, once unlocked, a Cattle Barn; tap a building to select it or hold and drag to reposition it. An incomplete cattle barn can move until it gains a pen or cattle; after that its position locks and only its fence remains editable
...
- automatic browser-local saving of the generated Farmipelago, field and crop state, buildings, cattle pens, individual herds, feed and milk stores, delivery progress, vehicle positions, loadouts, storage, and active seed; schema-6 farms migrate in place and refreshing restores the same playable state
...
- completed cattle pens receive two adult cows. Every four valid pasture tiles provide one hard herd-capacity slot; cows wander independently within the pen, calves arrive automatically when hay and capacity are available, and calves mature into milk-producing adults
...
- keyboard fallback: WASD/arrows to drive, Space to jump, Q/E for front/rear tools, V to cycle vehicles, F to cycle seed, B for construction, and Escape to leave a special view or open the menu. In build mode, tap a building to select it and hold-drag movable buildings; a cattle barn only moves before it has a pen or animals. Place a new barn and draw from one barn-side anchor around the pasture to the other; select fence corners or segments to edit them, or use Redraw selected pen. Nearby contextual popups transfer silo crops, feed a carried bale, load barn milk, and deliver eligible cargo
