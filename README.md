...
**Automatic Flour pallets (Step 6 review, build 0.375):** Pause → Debug → **Add 4 Flour pallets** enables saved **Debug · Workshop stock**. Drive the tractor into the Workshop, equip **Flatbed** (four slots, no unlock needed), then leave the bay and stop with the trailer within five tiles of the workshop doorway. Press **Load** in the building HUD: sacks on wooden pallets animate onto the deck and remain visible. Drive across the bridge to the Settlement Storehouse front yard; **Debug · Storehouse stock** offers **Unload** into its eight slots. Both fixtures support Load and Unload for repeat review. Buttons support touch or Tab + Enter/Space; **Cancel** stops an active transfer. Pause freezes it, and moving out of range, jumping or switching vehicles cancels it. Counts change only when each pallet arrives; reload retains completed units and returns the unfinished animation to its source inventory. A loaded flatbed cannot be unequipped.

Flour cannot enter silos, Grain Trailers, combines or seeds. Pallet Forks are unavailable and hay-bale handling is unchanged. Legacy bulk Flour converts at one pallet per 1,000 L (partial inventories round up); former manual Flour pallets, including those on released islands, recover into Workshop stock once. Debug receiving stock is separate from settlement progress. Windmill production, island opportunities and normal settlement pallet delivery remain future steps; Tier 2 Flour is still unavailable at a provisional four pallets.

**Manual review:** load four pallets and check the full/empty limits; add more Debug stock and fill the eight-slot receiver; reverse transfers to free space. Cancel or drive away mid-animation and refresh both during transfers and while parked: Workshop + Flatbed + Storehouse totals should stay constant except when explicitly adding Debug stock. Check trailer turning/reversing, saved alignment, vehicle switching and rescue; confirm crop transfers, hay feeding, keyboard driving/jump and phone controls still work. No automated gameplay checks were run.
...
roughly 20-second encounters (16–24-second targets, longer when routes are blocked), converging traffic, multi-bridge attachment, releases
